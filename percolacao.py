...
def box_counting(labels, perc_label, n):
    ...
    for b in range(0, max_pow):
        bs = 2 ** b
        boxes = set()
        for r, c in cells:
            boxes.add((r // bs, c // bs))
        nboxes_side = math.ceil(n / bs)
        results.append({'eps': 1.0 / nboxes_side, 'N': len(boxes)})
...
def fractal_dimension(box_data):
    ...
    Descarta a escala mais fina (efeito de pixel) e a mais grossa
    (poucas caixas, alta variância). O ajuste é feito apenas na
    região de escala intermediária.
    """
    if len(box_data) < 2:
        return None
    data = box_data[1:-1] if len(box_data) >= 4 else box_data
    if len(data) < 2:
        return None
